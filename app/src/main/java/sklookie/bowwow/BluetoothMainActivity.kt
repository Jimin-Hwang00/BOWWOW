...
import sklookie.bowwow.databinding.ActivityBluetoothMainBinding
import java.util.*
    ...
    private fun connectDevice(deviceAddress: String) {
        mBluetoothAdapter?.let { adapter ->
            ...
            try {
                ...
                thread.run()
                Toast.makeText(
                    this@BluetoothMainActivity, "${device.name}과 연결되었습니다." +
                            " 기기를 확인해주세요.", Toast.LENGTH_SHORT
                ).show()
            } catch (e: Exception) { // 연결에 실패할 경우 호출됨
                Toast.makeText(
                    this@BluetoothMainActivity, "기기의 전원이 꺼져 있습니다." +
                            " 기기를 확인해주세요.", Toast.LENGTH_SHORT
                ).show()
            }
        }
